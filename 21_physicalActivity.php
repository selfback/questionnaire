<?php
    ...
    class PhysicalActivity{
        ...
        public function preparedStack(&$tab){
            $temp = new stdClass();
            $temp->value = $this->activity;
            $tab = array_merge($tab, array("p".getPageNumber($this->pageIndex)."_q01" => $temp));
        }

        public function loadValue($tab){
            if(isset($tab["p".getPageNumber($this->pageIndex)."_q01"])) $this->activity = $tab["p".getPageNumber($this->pageIndex)."_q01"]->value;
        }
    }

    $user_response = (array)json_decode($_SESSION['user_response']);
    $noGood = false;
    $object = new PhysicalActivity($_SESSION['page_index']);
    $object->loadValue($user_response);
    
    if(!empty($_POST)){
        if(isset($_POST['activity']) && $_POST['activity'] != "") $object->activity = $_POST['activity'];
        else $noGood = true;
...
?>  
                    <p class="row css3-radio">
                        <input id="radio<?= $i ?>" type="radio" name="activity" value="<?= $i ?>" <?= $object->activity == "".$i ? "CHECKED" : "" ?> />
                        <label class="toggler_r" for="radio<?= $i ?>">
                            <?= constant("P21_ANSWER_".($i + 1)) ?>
                        </label>
                    </p>
<?php
    }
