<?php
    ...
    class WorkCharacteristics{
        ...
        public function preparedStack(&$tab){
            $temp = new stdClass();
            $temp->value = $this->jobDescribe;
            $tab = array_merge($tab, array("p".getPageNumber($this->pageIndex)."_q01" => $temp));
        }

        public function loadValue(&$tab){
            if(isset($tab["p".getPageNumber($this->pageIndex)."_q01"])) $this->jobDescribe = $tab["p".getPageNumber($this->pageIndex)."_q01"]->value;
        }
    }

    $user_response = (array)json_decode($_SESSION['user_response']);
    $noGood = false;
    $object = new WorkCharacteristics($_SESSION['page_index']);
    $object->loadValue($user_response);
    
    if(!empty($_POST)){
        if(isset($_POST['jobDescribe']) && $_POST['jobDescribe'] != "") $object->jobDescribe = $_POST['jobDescribe'];
        else $noGood = true;
...
?>                      
                    <p class="css3-radio">
                        <input id="radio<?= $i ?>" type="radio" name="jobDescribe" value="<?= $i ?>" <?= $object->jobDescribe == "".$i ? "CHECKED" : "" ?> />
                        <label class="toggler_r" for="radio<?= $i ?>">
                            <?= constant("P7_ANSWER_".($i + 1)) ?>
                        </label>
                    </p>
<?php
    }
