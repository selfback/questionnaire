<?php
    ...
    class Education{
        ...
        public function preparedStack(&$tab){
            $temp = new stdClass();
            $temp->value = $this->eductationBackground;
            $tab = array_merge($tab, array("p".getPageNumber($this->pageIndex)."_q01" => $temp));
        }

        public function loadValue($tab){
            if(isset($tab["p".getPageNumber($this->pageIndex)."_q01"])) $this->eductationBackground = $tab["p".getPageNumber($this->pageIndex)."_q01"]->value;
        }
    }

    $user_response = (array)json_decode($_SESSION['user_response']);
    $noGood = false;
    $object = new Education($_SESSION['page_index']);
    $object->loadValue($user_response);
    
    if(!empty($_POST)){
        if(isset($_POST['eductationBackground']) && $_POST['eductationBackground'] != "") $object->eductationBackground = $_POST['eductationBackground'];
        else $noGood = true;
...
?>
                    <p class="css3-radio">
                        <input id="radio<?= $i ?>" type="radio" name="eductationBackground" value="<?= $i ?>" <?= $object->eductationBackground == "".$i ? "CHECKED" : "" ?> />
                        <label class="toggler_r" for="radio<?= $i ?>">
                            <?= constant("P5_ANSWER_".($i + 1)) ?>
                        </label>
                    </p>
<?php
    }
